...
int info_imprimirClientePublicacion(Publicidad* array,int limitePubli, int idPublicidad,Cliente* arrayCli, int limiteCli)
{
	...
	if(array != NULL && limitePubli > 0 && idPublicidad >= 0 && arrayCli != NULL && limiteCli > 0)
	{
	for(i=0;i < limitePubli;i++)
	{
		...
		for(j=0;j<limiteCli;j++)
		{
			if(j == bufferAuxiliar)
			{
				printf("\nID: %d - Nombre: %s - Apellido: %s - Cuit: %s",arrayCli[j].id,arrayCli[j].nombre,
						arrayCli[j].apellido,arrayCli[j].cuit);
				break;
			}
		}
		}
...
/*
 * Brief Imprime la cantidad de publicaciones activas del rubro indicado
 * param array es el array de la publicidad
 * param limite es el limite maximo de busqueda en el array
 * return respuesta 0 (EXITO) -1 (ERROR)
 */
int info_cantidadDePublicacionesPorRubro(Publicidad* array, int limite)
{
	...
	printf("\nLa cantidad de publicaciones activas del Rubro %s es : %d",valorBuscado,contadorRubros);
	retorno = 0;
...
/*
 * Brief Imprime los datos del rubro con mas publicidades activas
 * param array es el array de la publicidad
 * param limite es el limite maximo de busqueda en el array
 * return respuesta 0 (EXITO) -1 (ERROR)
 */
...
/*
 * Brief Imprime los datos del rubro con menos publicidades activas
 * param array es el array de la publicidad
 * param limite es el limite maximo de busqueda en el array
 * return respuesta 0 (EXITO) -1 (ERROR)
 */
