...
static int esCuit(char* pResultado, int longitud);
 ...
 * \param mensajeError Es el mensaje de Error a ser mostrado
 * \param reintentos Cantidad de reintentos
 * \return Retorna 0 en caso de EXITO y -1 en casi de ERROR
 *
 */
...
static int esCuit(char* pResultado, int longitud)
{
	...
	{
		for(i=0;i<longitud && pResultado[i] != '\0';i++)
		{

			if((i==2 && pResultado[i] == '-' )||( i==11 && pResultado[i] == '-'))
			{
				continue;
			}
