	...
	do
	{
				...
				"\n13.Salir","\nOpcion invalida\n",1,13,3))
		{
			...
			case 3:
				...
				{
					pub_listaPublicacionesCliente(arrayPublicidades,CANTIDAD_PUBLICIDADES,buscarId);
			...
			case 4:
				...
				if(utn_getNumero(&buscarId,"\nIndique numero de ID de cliente\n","\nError, Id no existe\n",0,idCliente-1,2)==0 &&
					cli_buscarId(arrayClientes,CANTIDAD_CLIENTES,buscarId) !=-1)
				{
					pub_altaArray(arrayPublicidades,CANTIDAD_PUBLICIDADES,indiceAuxiliarPublicidad,&idPuclicidad,&buscarId);
